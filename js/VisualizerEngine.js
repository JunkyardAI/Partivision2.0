/* =========================================
   ...
   Optimized for centering and transitions
   ========================================= */
class VisualizerEngine {
    constructor(containerId) {
        ...
        // Ensure initial pan is dead-center
        this.camState = { mode: 'orbit', dist: 50, speed: 0.5, theta: Math.PI/4, phi: Math.PI/4, pan: new THREE.Vector3(0,0,0) };
    ...
    initVisuals() {
        const createMesh = (geo, mat) => { 
            ...
            m.position.set(0,0,0); // Explicitly centered
            this.scene.add(m); 
            m.visible = false; 
            return m; 
        };
        const createPoints = (geo, mat) => { 
            const p = new THREE.Points(geo, mat); 
            p.position.set(0,0,0); // Explicitly centered
            this.scene.add(p); 
            p.visible = false; 
            return p; 
        };
